...
function Swatch({ name, hex, usage, note }) {
  return (
    <div className="flex items-start gap-3 p-3 rounded-lg" style={{ background: color.card }}>
      <div
        className="w-12 h-12 rounded-md shrink-0 border"
        style={{ background: hex, borderColor: color.rule }}
      />
      <div>
        <div className="text-sm font-semibold" style={{ ...fontBody, color: color.ink }}>
          {name}
        </div>
        <div className="text-xs" style={{ ...fontMono, color: color.pencil }}>
          {hex}
        </div>
        <div className="text-xs mt-1" style={{ ...fontBody, color: color.pencil }}>
          {usage}
        </div>
        {note && (
          <div className="text-xs mt-1 font-semibold" style={{ ...fontBody, color: color.pesto }}>
            {note}
          </div>
        )}
      </div>
...
function SectionLabel({ children }) {
  return (
    <div
      ...
      style={{ ...fontMono, color: color.enamel }}
    >
      {children}
    </div>
  )
...
function Button({ children, variant = 'primary' }) {
  const styles = {
    ...
    ghost: {
      ...
      color: color.enamel,
      border: `1.5px solid ${color.enamel}`,
    },
    ...
    <div className="min-h-screen p-6" style={{ background: color.kraft }}>
      ...
      <div className="max-w-3xl mx-auto mb-10">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <div className="text-3xl font-extrabold" style={{ ...fontDisplay, color: color.ink }}>
              Chessy
            </div>
            <div className="text-lg -mt-1" style={{ ...fontHand, color: color.tomato }}>
              Your home, handled.
            </div>
          </div>
        </div>
      ...
      <div className="max-w-3xl mx-auto mb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          ...
          <Swatch
            name="Pencil"
            hex={color.pencil}
            usage="Secondary text, on Card only — 5.6:1"
            note="AA"
          />
          <Swatch
            name="Enamel"
            hex={color.enamel}
            usage="Primary buttons / nav, white text — 8.7:1"
            note="AAA"
          />
          <Swatch
            name="Mustard"
            hex={color.mustard}
            usage="Chessy's picks, ink text — 6.1:1"
            note="AA"
          />
          ...
          <Swatch
            name="Pesto"
            hex={color.pesto}
            usage="In-stock / success, white text — 6.0:1"
            note="AA"
          />
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto mb-10">
        ...
        <div className="rounded-lg p-5 space-y-4" style={{ background: color.card }}>
          <div>
            <div className="text-xs mb-1" style={{ ...fontMono, color: color.pencil }}>
          ...
          <div>
            ...
            <div className="text-base" style={{ ...fontBody, color: color.ink }}>
              Used for descriptions, instructions, and everyday UI text. Warm, rounded, easy to read
              on a phone screen.
            </div>
          </div>
          <div>
            <div className="text-xs mb-1" style={{ ...fontMono, color: color.pencil }}>
              LABEL / DATA — Space Mono
            </div>
          ...
          <div>
            <div className="text-xs mb-1" style={{ ...fontMono, color: color.pencil }}>
              ACCENT — Caveat (Chessy's handwriting)
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <RecipeCard accent={color.tomato}>
            <div className="flex justify-between items-start mb-2">
              <div className="text-lg font-bold" style={{ ...fontDisplay, color: color.ink }}>
            ...
            <div className="flex gap-3 text-xs mb-2" style={{ ...fontMono, color: color.pencil }}>
              <span className="flex items-center gap-1">
                <Clock size={12} /> 35 min
              </span>
              <span className="flex items-center gap-1">
                <Snowflake size={12} /> Thaw chicken
              </span>
            </div>
            <p className="text-sm" style={{ ...fontBody, color: color.ink }}>
              Peppers, onion, chicken thighs, fajita seasoning. Kid-approved — Emma will eat the
              peppers if they're soft.
            </p>
          </RecipeCard>

          <RecipeCard accent={color.enamel}>
            <div className="text-lg font-bold mb-2" style={{ ...fontDisplay, color: color.ink }}>
            ...
            <ul className="space-y-2 text-sm" style={{ ...fontMono, color: color.ink }}>
              <li className="flex items-center gap-2">
                <span
                  ...
                  style={{ borderColor: color.enamel }}
                />{' '}
                Chicken thighs (2 lb)
              </li>
              <li className="flex items-center gap-2">
                <span
                  ...
                  style={{ background: color.pesto }}
                >
                  <Check size={10} color="#fff" />
                </span>
                <span className="line-through" style={{ color: color.pencil }}>
                  Bell peppers
                </span>
              </li>
              <li className="flex items-center gap-2">
      ...
      <div className="max-w-3xl mx-auto mb-10">
        ...
        <div className="rounded-lg p-5 space-y-3" style={{ background: 'transparent' }}>
          ...
          <ChessyBubble>
            Added chicken thighs to this week's list — and reminding you to pull them from the
            freezer tonight.
          </ChessyBubble>
          <ChessyBubble>
            We're down to our last roll of paper towels. I added it to the list.
          </ChessyBubble>
          <ChessyBubble>
            I couldn't read that recipe link — paste the ingredients in and I'll take it from there.
          </ChessyBubble>
          <ChessyBubble>
            Nothing in the recipe box yet. Send me one you love and I'll keep it safe.
          </ChessyBubble>
        </div>
        <p className="text-xs mt-2 px-1" style={{ ...fontBody, color: color.pencil }}>
          Warm, capable, plain-spoken. Never apologizes for errors — explains what happened and
          what's next. Confirms actions in the same words used to request them.
        </p>
      </div>
          ...
          <div className="flex justify-between rounded-lg p-2" style={{ background: color.enamel }}>
            {[
              ...
              <div
                ...
                style={{ color: i === 0 ? color.mustard : '#fff' }}
              >
                <Icon size={18} />
                <span className="text-[10px]" style={{ ...fontBody }}>
                  {label}
                </span>
              </div>
            ))}
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div
          ...
          style={{ background: color.card, ...fontBody, color: color.ink }}
        >
          <p>
            ✅ <strong>Contrast checked</strong> — every text/background pairing above meets WCAG AA
            (4.5:1) or AAA. Tomato and Mustard are reserved for badges/icons with the paired text
            colors shown, not for body text directly on Kraft.
          </p>
          <p>
            ✅ <strong>Secondary text rule</strong> — Pencil (#6E6258) is AA on Card but falls just
            short on Kraft. Rule: secondary text only appears on Card surfaces, never directly on
            the page background.
          </p>
          <p>
            ⚠️ <strong>Font loading</strong> — for offline PWA use, these four fonts need to be
            self-hosted (bundled in the build) rather than loaded via Google Fonts CDN. Flagging for
            Phase 0b build setup.
          </p>
          <p>
            ⚠️ <strong>Motion</strong> — none proposed yet by design; will respect
            prefers-reduced-motion when we add any transitions in Phase 1.
          </p>
        </div>
      </div>
    </div>
